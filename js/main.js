$(document).ready(function () {
  ...
  $($gallery.find("ul")).swipe({
    swipe: function (
      ...
      if (direction == "left") {
        ...
        if (-move == -img_width * img_length) {
          // 마지막 사진까지 넘기고 한번 더 넘길 때
        ...
        // 방향 전환할 때(만), move값이 img_width만큼 더 가야하므로 제어필요
        if (
          (status == "left side" && -move !== -img_width * img_length) ||
          (status == "left side" && -move !== -img_width * (img_length - 1))
        ) {
          // 현재 상태가 왼쪽 방향이면서 마지막사진이 아닐 때
          status = "right side"; // 오른쪽 방향(상태)로 바꾸기
          move -= img_width;
          // console.log('status: ',status);
        } else if (status == "left side" && -move == -img_width * img_length) {
          // 마지막 이미지일 때,
          status = "right side";
          // console.log('status: ',status);
        }
  ...
  $(".pignose-calendar.pignose-calendar-light.pignose-calendar-default").append(
    `<div class="pignose-calendar-button-group"><a href="#" class="pignose-calendar-button pignose-calendar-button-cancel">Cancel</a><a href="#" class="pignose-calendar-button pignose-calendar-button-apply">OK</a></div>`
  );
